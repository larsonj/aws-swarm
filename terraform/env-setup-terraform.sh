...
export AWS_KEY_NAME="/.ssh/devops21.pem"

#################################
# Terraform Config
#################################
export TF_VAR_vpc_id=$AWS_DEFAULT_VPC_ID
export TF_VAR_subnet_id=subnet-277b026e

export TF_VAR_swarm_manager_token=$(ssh -i $AWS_KEY_PATH \
   ubuntu@$(terraform output swarm_manager_1_public_ip) \
   docker swarm join-token -q manager)

export TF_VAR_swarm_worker_token=$(ssh -i $AWS_KEY_PATH \
   ubuntu@$(terraform output swarm_manager_1_public_ip) \
